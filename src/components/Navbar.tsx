            ...
            {[
              ...
              <React.Fragment key={item.label}>
                <Magnetic intensity={0.15}>
                  <Link
                    to={item.path}
                    className={`text-xs uppercase tracking-[0.15em] transition-colors duration-300 relative group block ${(isScrolled || location.pathname !== "/") ? 'text-alabaster-muted hover:text-white' : 'text-white/80 hover:text-white'}`}
                  >
                    {item.label}
            ...
            <div className="flex flex-col items-center gap-8 z-10">
              ...
              >
                <Link
                  to="/#contact"
                  onClick={() => setMobileMenuOpen(false)}
                  className="px-8 py-4 border border-champagne text-champagne text-sm uppercase tracking-widest hover:bg-champagne hover:text-obsidian transition-colors block"
                >
